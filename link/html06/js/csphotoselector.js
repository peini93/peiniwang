	...
	newInstance = function(options) {
		...
		bindEvents = function() {
			$buttonClose.bind('click', function(e) {
				e.preventDefault();
				hideAlbumSelector();
				flashMovie("flashcontent").jsBack("");
	...
	buildAlbumSelector = function(id, callback) {
		...
		buildAlbumMarkup = function(album, accessToken) {
			return '<a href="#" class="CSPhotoSelector_album" data-id="' + album.id + '">' +
					...
					'<img src="https://graph.facebook.com/'+ album.id +'/picture?type=album&access_token='+ accessToken +'" alt="' + album.name.replace("Mobile Uploads", "手機上傳") + '" class="CSPhotoSelector_photoAvatar" />' +
					'</div></div>' +
					'<div class="CSPhotoSelector_photoName">' + album.name.replace("Mobile Uploads", "手機上傳") + '</div>' +
					'</a>';
